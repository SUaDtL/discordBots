...
//
// Standalone deploy-commands script (AC-07). Registers /post-role-menu idempotently via bot-core's
// `registerGuildCommands` (lists existing commands first, then PUTs only when something is missing).
// The testable core (`deployCommands`) takes an injected REST so tests never hit Discord; `main()` is
// guarded so importing this file does NOT perform network calls.
...
import { loadArbiterSelfAssignConfig } from './config.js';
...
/** Idempotently register arbiter-self-assign's slash commands into the guild. */
...
async function main(): Promise<void> {
  ...
  console.log(
    `arbiter-self-assign: registered [${result.registered.join(', ')}], skipped [${result.skipped.join(
      ', ',
    )}]`,
  );
}
